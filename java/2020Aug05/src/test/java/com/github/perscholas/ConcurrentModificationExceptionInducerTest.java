 ...
 */
public class ConcurrentModificationExceptionInducerTest implements Sleeper {
    public static void main(String[] args) {
        new ConcurrentModificationExceptionInducerTest().testJoiningThread2();
    }

    private void testSleeping() {
        /** Given */
        // given there are two threads
        Thread thread1 = new Thread(new ConcurrentModificationExceptionInducer());
        Thread thread2 = new Thread(new ConcurrentModificationExceptionInducer());

        // given the ending value is `${some-value}`
        int expectedEndingValue = ConcurrentModificationExceptionInducer.getEndValue();

        // given the expected starting value is `0`
        int expectedStartValue = 0;
        int actualStartingValue = ConcurrentModificationExceptionInducer.getCurrentValue();
        ...
        /** When */
        int actualEndingValue = ConcurrentModificationExceptionInducer.getCurrentValue();
